...
describe("MemoryManager API calls", () => {
  ...
  it("requests tags and merge advice", async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({ verdict: "merge!" }),
      });

    const { getAllByText, getAllByPlaceholderText, getByText, findByText } =
      renderWithUser(fetchMock);

    fireEvent.change(getAllByPlaceholderText("Content")[0], {
      target: { value: "foo" },
    });
    fireEvent.click(getByText("Suggest Tags"));
    await waitFor(() => {
      expect(fetchMock).toHaveBeenCalledWith(
        expect.stringContaining("/ai/tags"),
        expect.any(Object),
      );
    });
    const tagCall = fetchMock.mock.calls.find((c) =>
      (c[0] as string).includes("/ai/tags"),
    );
    expect(tagCall).toBeTruthy();
    if (tagCall) {
      expect(JSON.parse((tagCall[1] as RequestInit).body as string)).toEqual({
        text: "foo",
      });
    }
      ...
      expect(
        fetchMock.mock.calls.some(
          (c) =>
            ...
            JSON.parse((c[1] as RequestInit).body as string).memory_ids[0] ===
              1,
        ),
      ).toBe(true);
    });
